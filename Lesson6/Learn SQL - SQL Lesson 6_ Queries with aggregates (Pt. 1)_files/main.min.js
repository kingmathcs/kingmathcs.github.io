...
literal:"true false",built_in:"array bigint binary bit blob boolean char character date dec decimal float int integer interval number numeric real serial smallint varchar varying int8 serial8 text"},c:[{cN:"string",b:"'",e:"'",c:[a.BE,{b:"''"}]},{cN:"string",b:'"',e:'"',c:[a.BE,{b:'""'}]},{cN:"string",b:"`",e:"`",c:[a.BE]},a.CNM,a.CBCM,c]},a.CBCM,c]}});Date.now||(Date.prototype.now=function(){return(new Date).getTime()});
var GA_PAGE_KEY,FOCUS_CONTINUE_BUTTON_ON_COMPLETE=!1,ALLOW_SCROLL_RO_DATATABLES=!0,DEBUG=!1;function Db(a){this.db=new SQL.Database(a)}Db.prototype.query=function(a){if(DEBUG)var c=Date.now();var b=null,d=null;try{b=this.db.exec(a)[0]}catch(e){d=e}DEBUG&&console.log("Query: "+a,Date.now()-c);return new DbResult(b,d)};Db.prototype.close=function(){this.db.close()};
...
function Exercise(a,c,b){this.db=c;this.data=b;this.exerciseJqEl=a;this.tableJqEls=a.find(".datatable");this.tableRenderers=[];this.tasks=[];this.tasksListJqEl=a.find(".tasks_list");this.clearJqEl=a.find(".clear");this.editorJqEl=a.find(".sqlinput");this.editorTargetDatatableId=this.editorJqEl.attr("targetDatatableId");this.editorTargetDatatableRenderer=null;this.editor=ace.edit(this.editorJqEl[0]);this.editorLastSqlQuery=this.editorInputTimeoutTimer=null;this.solutionJqEl=a.find(".solution_trigger");
this.continueJqEl=a.find(".continue");this.messageJqEl=a.find(".message");this.messageTimeoutTimer=null}Exercise.prototype.proxy=function(a){return $.proxy(a,this)};Exercise.prototype.initialize=function(){this.initializeTables();this.initializeTasks();this.initializeEditor()};
Exercise.prototype.initializeTables=function(){this.tableJqEls.each(this.proxy(function(a,c){if(DEBUG)var b=Date.now();var d=$(c).attr("datatableId"),e=new DbTableRenderer,f=this.data.datatable[d].preloadTableAction;if(f)if("randomizeRows"==f.action){var l=this.db.query("SELECT * FROM "+f.column+" ORDER BY RANDOM()");this.db.query("DELETE FROM "+f.column);$.each(l.rows,this.proxy(function(a,b){b._values[0]=a+1;this.db.query('INSERT INTO movies VALUES ("'+b._values.join('","')+'");')}))}else alert("Invalid preload action");
else(f=this.data.datatable[d].preloadTableQuery)&&(this.db.query(f)||alert("Invalid preload queries"));this.editorLastSqlQuery=this.data.datatable[d].defaultQuery;f=this.db.query(this.editorLastSqlQuery);e.initialize(f);e.setRenderOptions($(c));e.select(f);this.tableRenderers.push(e);this.editorTargetDatatableId==d&&(this.editorTargetDatatableRenderer=e);f.errorMessage?this.showMessage(f.errorMessage,"error",null):this.hideMessage();DEBUG&&console.log("initializeTable: "+a,Date.now()-b)}))};
Exercise.prototype.initializeTasks=function(){if(DEBUG)var a=Date.now();$.each(this.data.tasks,this.proxy(function(a,b){this.tasks.push(new Task(this.db,"queryChecks"in b?b.queryChecks:null,b.checks,b.solution,b.userSolution,b.postValidateAction))}));$(this.tasksListJqEl.children("li")[0]).addClass("active");this.solutionJqEl.on("click",this.proxy(function(a){for(var b=null,d=0,e=0;e<this.tasks.length;e++)if(b=this.tasks[e],!b.isSatisfied()){d=e;break}ga("send","event","solution","click",GA_PAGE_KEY+
"/"+(d+1));this.editor.setValue(b.getUserSolution());this.tableJqEls.scrollTop(0);a.preventDefault();return!1}));this.continueJqEl.on("click",this.proxy(function(a){var b=!0;$.each(this.tasks,this.proxy(function(a,c){b=b&&c.isSatisfied()}));return b?!0:(a.preventDefault(),!1)}));0==this.tasks.length&&this.enableContinueButton();this.clearJqEl.on("click",this.proxy(function(a){this.editor.setValue(this.data.datatable[this.editorTargetDatatableId].defaultQuery);a.preventDefault();return!1}));DEBUG&&
console.log("initializeTasks",Date.now()-a)};
Exercise.prototype.initializeEditor=function(){if(DEBUG)var a=Date.now();this.editor.renderer.setShowGutter(!1);this.editor.renderer.setPrintMarginColumn(!1);this.editor.renderer.setShowPrintMargin(!1);this.editor.renderer.setPadding(16);this.editor.setHighlightActiveLine(!1);this.editor.getSession().setMode("ace/mode/sql");this.editor.getSession().setUseWrapMode(!0);this.editor.setValue(this.data.inputDefaultQuery);this.editor.clearSelection();this.editor.blur();this.editor.on("change",this.proxy(function(){this.requestUpdateFromSql()}));
if(window.innerHeight){var c=this.editorJqEl.offset().top,b=$(window);b.on("scroll",this.proxy(function(){var a=b.scrollTop();a<=c&&c<=a+window.innerHeight&&this.editor.focus()}))}this.editor.commands.removeCommand("gotoline");this.editor.commands.removeCommand("find");this.editor.commands.removeCommand("findnext");DEBUG&&console.log("initializeEditor",Date.now()-a)};
...
Exercise.prototype.updateFromSql=function(){if(DEBUG)var a=Date.now();var c=$.trim(this.editor.getValue());if(c!=this.editorLastSqlQuery)if(this.editorLastSqlQuery=c,0==c.length){for(var a=this.data.inputDefaultQuery,b=this.tasks.length-1;0<=b;b--)if(this.tasks[b].isSatisfied()&&this.tasks[b].hasPostValidateAction()){a=this.tasks[b].getPostValidateQuery();break}b=this.db.query(a);this.editorTargetDatatableRenderer.select(b)}else{for(var d=null,e=-1,b=0;b<this.tasks.length;b++)if(!this.tasks[b].isSatisfied()){d=
this.tasks[b];e=b;break}if(d&&!d.verifySqlInput(c))this.showMessage("Awaiting complete query...",null,null);else if(b=this.db.query(c),b.errorMessage)this.showMessage(b.errorMessage,"error",null);else{this.hideMessage();this.editorTargetDatatableRenderer.select(b);if(d&&d.verify(c,b)){c=$(this.tasksListJqEl.children("li")[e]);c.html("<span class='completed'>"+c.html()+" <span class='check'>&#10003;<br>Solution: " + ace.edit("ace_text").getValue() + "</span></span>");c.find(".hint").remove();e<this.tasks.length-1&&(c=$(this.tasksListJqEl.children("li")[e+
1]),c.addClass("active"));if(this.tasks[e].hasPostValidateAction()&&((c=this.tasks[e].getPostValidateActionRunOnce())&&this.db.query(c),c=this.tasks[e].getPostValidateQuery())){var f=this.tasks[e].getPostValidateMessage(),b=this.db.query(c);this.editorTargetDatatableRenderer.select(b);setTimeout(this.proxy(function(){this.showMessage(f,null,2E3,function(){this.editor.setValue("");this.editor.clearSelection()})}),100)}e==this.tasks.length-1&&this.enableContinueButton()}DEBUG&&console.log("updateFromSql - preUpdateTables",
Date.now()-a);this.tableJqEls.each(this.proxy(function(a,b){var c=$(b).attr("datatableId");this.editorTargetDatatableId!=c&&(c=this.db.query(this.data.datatable[c].defaultQuery),this.tableRenderers[a].select(c))}));DEBUG&&console.log("updateFromSql",isFirstLoad,Date.now()-a)}}};Exercise.prototype.enableContinueButton=function(){this.continueJqEl.removeClass("disabled");this.continueJqEl.html("Finished! 👏 Click to return home <span>&rsaquo;</span>");FOCUS_CONTINUE_BUTTON_ON_COMPLETE&&this.continueJqEl.focus()};
...
function initializeDocumentLoad(a,c){GA_PAGE_KEY=c;new ButtonHoverHelper($("body"),$(".menu_container"),$(".lesson_button"),$(".topics_button"));ALLOW_SCROLL_RO_DATATABLES&&!Modernizr.touchevents&&new DatatableScrollHelper($(".datatable.read_only"),"hidden",500);$(".paypal_donation_button").bind("click",function(a){ga("send","event","paypal_donation","click",GA_PAGE_KEY);$("#paypal_form").submit();a.preventDefault();a.stopPropagation()});$(".lesson .body code[class=sql]").each(function(a,b){hljs.highlightBlock(b)});
var b="undefined"===typeof disableExercises;if(a&&b){if(DEBUG)var d=Date.now();var e=new Db(dbData),f=JSON.parse(a);$(".exercise").each(function(a,b){(new Exercise($(b),e,f)).initialize()});f=e=null;DEBUG&&console.log("Building Exercise",Date.now()-d)}else b||($(".exercise").hide(),$(".disabled_exercise_overlay").show())};
